...
export class AppointmentService {
  ...
  async getAllAppointments(paginationDto: PaginationAndFilterDto, filters: any) {
    ...
    // تحقق إذا كان يوجد نص للبحث في الأسماء (name of doctor, patient)
    if (filters.search) {
    ...
    const result = await paginate(
      ...
      [
        ...
        { path: 'patient', select: 'name' }, // Populate patient name based on patient ID
        { path: 'clinic', select: 'name' },  // Populate clinic name based on clinic ID (optional)
      ],
      page,
      limit,
      allData,
